...
namespace ERPServer.Application.Features.Productions.DeleteProductionById
{
        ...
        IUnitOfWork unitOfWork) : IRequestHandler<DeleteProductionByIdCommand, Result<string>>
    {
        ...
        {
            Production? production = await productionRepository.GetByExpressionAsync(f => f.Id == request.Id, cancellationToken);
            //
            ...
            //
            List<StockMovement> movements = await stockMovementRepository.Where(f => f.ProductionId == production.Id)
