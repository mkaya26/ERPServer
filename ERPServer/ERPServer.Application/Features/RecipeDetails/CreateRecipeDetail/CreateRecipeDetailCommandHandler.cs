...
namespace ERPServer.Application.Features.RecipeDetails.CreateRecipeDetail
{
        ...
        IMapper mapper) : IRequestHandler<CreateRecipeDetailCommand, Result<string>>
    {
        ...
        {
            var recipeDetail = await recipeDetailRepository.GetByExpressionWithTrackingAsync(f => f.RecipeId == request.RecipeId &&
            f.ProductId == request.ProductId, cancellationToken);
            //
            ...
            //
            return "Reçete ürün kaydı başarıyla gerçekleşti.";
        }
    }
}
